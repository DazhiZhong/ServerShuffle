...
def get_dimensions(requested_size_in_kilobytes):
    ...
    axis = int(math.sqrt(size_in_bytes/3))
...
def generate_random_image_by_size(requested_size_in_kilobytes, monochrome=False):
    ...
    dimensions = get_dimensions(requested_size_in_kilobytes)
    for x in range(0, dimensions['total_pixels']):
        if monochrome:
...
def generate(monochrome=False,output_path='image.png',randomize=True,requested_size=200):
    """
    ...
    # parser.add_argument('--size', '-s', dest="requested_size", type=int, help="Requested size, in kilobytes, of image")
    ...
    # print(args)
    # rargs={'monochrome':False,'output_path':'image.png','randomize':True,'requested_size':200}

    args=ns(**{'monochrome':monochrome,'output_path':output_path,'randomize':randomize,'requested_size':requested_size})
